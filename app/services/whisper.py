...
def transcribe_audio(audio_path: Path) -> Dict[str, Any]:
    ...
    with audio_path.open("rb") as f:
        ...
        try:
            resp = httpx.post(settings.WHISPER_URL, files=files, timeout=timeout)
        except httpx.RequestError as exc:
            raise WhisperError(f"Whisper request failed: {exc}") from exc

    if resp.status_code >= 400:
        raise WhisperError(f"Whisper error {resp.status_code}: {resp.text[:200]}")

    data = resp.json()
    text = data.get("text") or data.get("transcript") or data.get("transcriptText")
    if not text:
